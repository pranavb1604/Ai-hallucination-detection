...
# Reads raw HaluEval and TruthfulQA CSVs, unifies them into a single
# schema, then writes train.csv and test.csv to data/processed/.

# Unified schema:
#     question  : str   — the input question
#     answer    : str   — the LLM-generated (or benchmark) answer
#     label     : int   — 1 = hallucinated, 0 = correct/grounded
#     source    : str   — "halueval" or "truthfulqa"
...
#     """
#     Each row in HaluEval has both a right_answer and a hallucinated_answer.
#     We expand each row into two rows — one correct (label=0), one hallucinated (label=1).
#     """
#     df = pd.read_csv(path)
#     records = []
...
#         hallucinated = str(row.get("hallucinated_answer", "")).strip()

#         if right:
#             records.append({"question": q, "answer": right,       "label": 0, "source": "halueval"})
#         if hallucinated:
#             records.append({"question": q, "answer": hallucinated, "label": 1, "source": "halueval"})

#     result = pd.DataFrame(records)
#     print(f"  HaluEval: {len(result)} rows (after expansion)")
#     return result
...
# def _safe_parse(val):
#     """Safely parse a string that looks like a Python list/dict."""
...
#     """
#     TruthfulQA multiple_choice columns:
#         question, mc1_targets, mc2_targets
#     mc1_targets / mc2_targets are dicts:
#         {"choices": [...], "labels": [...]}   (label 1 = correct, 0 = incorrect)

#     We use mc1_targets for simplicity (single correct answer format).
#     Each choice becomes one row.
#     """
#     df = pd.read_csv(path)
#     records = []
...
#             if not answer:
#                 continue
#             # TruthfulQA label: 1 = correct → we invert to hallucination label
#             records.append({
#                 "question": q,
#                 "answer":   answer,
#                 "label":    0 if lbl == 1 else 1,   # 0=correct, 1=hallucinated
#                 "source":   "truthfulqa",
#             })

#     result = pd.DataFrame(records)
#     print(f"  TruthfulQA: {len(result)} rows (after expansion)")
#     return result
